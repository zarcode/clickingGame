...
class Board extends Component {
    ...
    fieldClick = (field, isPossible) => {
        ...
        if(!isPossible && this.state.started)
            return false;
        ...
        const possible = this.boardApi.getPossibleMovements(field)
        ...
        return (
            <div className="board">
                {this.board.map((x) =>
                    this.board.map((y) => {
                            const field = [x,y];
                            const isPossible = this.boardApi.isFieldInArray(field, this.state.possible);
                            ...
                            const classes =
                                (this.boardApi.isFieldInArray(field, this.state.generated)?"passive ":"") +
                                (this.state.generated.length === 0?"initial ":"") +
                                (isPossible?"possible ":"") +
                                (this.boardApi.isFieldInArray(field, this.state.selected)?"selected ":"");
